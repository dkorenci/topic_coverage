...
#TODO make createId main implementation for constructing ids
#TODO       and make IdComposer its clint
#TODO ? enable client to specify specific order attributes in the id
...
def autoId(components):
    '''
    Construct a fixed unique id from a list of components's ids.
    :param components: list of objects, each having an 'id' property
    '''
    from hashlib import pbkdf2_hmac
    ordids = sorted([str(c.id) for c in components])
    ordids= ';'.join(ordids)
    h = pbkdf2_hmac('sha512', ordids, 'autoid_hash_salt', 10000, dklen=50)
    h = ''.join('%d'%ord(b) for b in h) # transform hash to a sequence of digits
    hid = 'autoId_%s' % h
    return hid