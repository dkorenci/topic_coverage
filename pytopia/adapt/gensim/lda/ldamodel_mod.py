...
# -*- coding: utf-8 -*-
#
# This code is derived from gensim ldamolde, licensed as:
#
...
import os
from pytopia.adapt.gensim.lda.label import modelLabel
...
class LdaModel(interfaces.TransformationABC):
    ...
    def __init__(self, corpus=None, num_topics=100, id2word=None,
                 ...
                 alpha='symmetric', alpha_init=None, eta=None, decay=0.5, offset=1.0,
                 eval_every=10, iterations=50, gamma_threshold=0.001, seed = None,
                 eval_passes = None, eval_results = None):
        """
        ...
        self.eval_passes = eval_passes; self.eval_results = eval_results
        ...
        if alpha == 'symmetric' or alpha is None:
            ...
            #self.alpha = numpy.asarray([1.0 / num_topics for i in xrange(num_topics)])
            self.alpha = numpy.asarray([alpha_init for i in xrange(num_topics)])
    ...
    def update(self, corpus, chunksize=None, decay=None, offset=None,
        ...
        for pass_ in xrange(passes):
            ...
            if self.eval_passes is not None:
                self.evaluateOnPass(pass_)
                self.saveCurrentState(pass_)
        #endfor entire corpus update

    def evaluateOnPass(self, passNo):
        'evaluate model using eval functions provided as constructor arguments'
        evals = {}
        print 'eval on pass %d for model %s' % (passNo, modelLabel(self))
        for f in self.eval_passes:
            val = f(self); evals[str(f)] = val
            print str(f) + ' : ' + '%.3f' % val
        self.eval_results.append(evals)

    def saveCurrentState(self, passNo, models_folder):
        # todo: add models_folder to model options
        folder = models_folder + modelLabel(self)+ '_at_pass_%d/'%passNo
        if not os.path.exists(folder): os.makedirs(folder)
        self.save(folder+'gensim_model')
