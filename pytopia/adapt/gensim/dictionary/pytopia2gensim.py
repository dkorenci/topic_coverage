...
def gdictCreateCompareTest(dict_):
    from pytopia.adapt.gensim.dictionary.GensimDictAdapter import GensimDictAdapter
    dict_ = resolve(dict_)
    gdict = pytopia2gensimDict(dict_)
    pytgdict = GensimDictAdapter(gdict)
    assert dict_ == pytgdict

def testDictConversion():
    # todo add more dictionaries to pytopia.testing and this test
    from pytopia.testing import setup
    testDicts= ['us_politics_dict']
    for d in testDicts:
        gdictCreateCompareTest(d)
