from doc_topic_coh.dataset.topic_splits import allTopics, devTestSplit2
from doc_topic_coh.dataset.topic_features import tableParse
...
def allTopicsNoMix():
    topics = selectTopics(allTopics(), ['theme', 'theme_noise', 'noise'])
    return [ (t, topicThemes(t, l)) for t, l in topics ]

def allTopicsMixedEmpty():
    return [ (t, topicThemes(t, l, emptyMixed=True)) for t, l in allTopics() ]
