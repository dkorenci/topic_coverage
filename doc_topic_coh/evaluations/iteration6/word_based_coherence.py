...
def palmettoOptUspolParamsPerType(type):
    ...
    windows = { 'index':'uspol_palmetto_index', 'windowSize': wsizes, 'topWords':[10,20] }
    p = IdList(jp([all], fp(windows)))
    p.id = 'palmetto_uspol_params_%s' % type
    return p

from doc_topic_coh.evaluations.iteration6.doc_based_coherence import dev, test
from doc_topic_coh.evaluations.iteration6.doc_based_coherence import expFolder
...
if __name__ == '__main__':
    ...
    #experiment(palmettoOptUspolParams(), action='run')
    experiment(palmettoOptUspolParams(), action='run')
