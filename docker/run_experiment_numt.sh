...
cacheFolder=$4 # folder for function's cache, has to be mounted
corpus=$5
covfunc=$6
modeltyp=$7
### SCRIPT PARAMS ###
...
runid="$runimage[$timestamp][$rndstamp][$corpus-$covfunc-$modeltyp]"
...
mount="$mount -v $cacheFolder:$cacheFolder"
...
echo "python -OO -u $scriptFile $modelFolder $cacheFolder $corpus $covfunc $modeltyp > $scriptFile.out" >> $entrypoint
